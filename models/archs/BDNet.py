...
class Fusion(nn.Module):
    ...
    def __init__(self, nf=64, nframes=3, center=2): # Tom: nframes=5 --> 3
        ...
        if DEBUG:
            print('class Fusion: nframes', nframes, 'nf', nf)
        self.fea_fusion = nn.Conv2d(nframes * nf, nf, 1, 1, bias=True)
        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=False)

    def forward(self, aligned_fea):
        if DEBUG:
            print('aligned_fea 0',  aligned_fea.shape)
        ...
        if DEBUG:
            print('aligned_fea',  aligned_fea.shape)
        fea = self.lrelu(self.fea_fusion(aligned_fea))
        return fea


class BDNet_train(nn.Module):
    def __init__(self, nf=64, nframes=3, groups=8, front_RBs=5, back_RBs=10, center=None,is_noise=True):
    ...
    def forward(self, x,noise,flag='syn'):
        ...
        if DEBUG:
            print('aligned_fea 3:', aligned_fea.shape)
